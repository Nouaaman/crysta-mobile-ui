...
const Upscale = () => {
   ...
   return (
      <SafeAreaView className="flex-1 bg-primary">
         <Image
            source={images.topRightRadialGradient}
            resizeMode="cover"
         ...
         <View className="p-4">
            <ToolsHeader
               title="Upscale"
               subtitle="Transform your low-resolution images into high-quality visuals."
               goBack={() => router.back()}
            />
         </View>
         <Animated.View
            ...
            <View className="flex-col w-full mt-8 p-4 pb-5 border-t-2 border-purple-400/10">
               <Text className="px-2 mb-4 text-xl text-textBody font-psemibold">
                  Preset:{" "}
                  {
                     UPSCALE_FACTOR.find(
                        (option) =>
                           option.value === selectedUpscaleFactor,
                     )?.label
                  }{" "}
                  •{" "}
                  {
               ...
               <View className="flex-row items-center gap-4 justify-stretch w-full">
                  ...
                  <ActionButton
                     title="Upscale"
                     handlePress={handleUpscalePress}
                  />
               </View>
         ...
         {sheetIsOpen && (
            <BottomSheet toggleSheet={toggleSheet}>
               <View className="flex-row items-center justify-between mb-6">
                  <Text className="text-3xl text-textBody font-psemibold">
                     Options
                  </Text>
                  <TouchableOpacity
                     onPress={toggleSheet}
               ...
               <View>
                  <View className="mb-8">
                     <Text className="text-xl text-textBody font-pbold">
                        Upscaling Factor
                     </Text>
                     <Text className="text-textBody">
                        Choose how much you want to enlarge the image
                     </Text>
                     <Radio
                        options={UPSCALE_FACTOR}
                        checkedValue={selectedUpscaleFactor}
                        onChange={(value) =>
                           setSelectedUpscaleFactor(value)
                        }
                        containerStyle="flex-row gap-3 mt-4 items-center justify-center"
                     />
                  </View>
                  <View className="mb-9">
                     <Text className="text-xl text-textBody font-pbold mb-2">
                        Processing Mode
                     </Text>
                     <Radio
                        options={MODELS}
